...
//----------------------//
    ...
    public Shift getShift(int day, int week, int employee) {
        try {
            this.enstablishConnection();
            ...
            String queryString = "SELECT * " +
                    "FROM Turnation " +
                    "WHERE (Day = ? ) AND (EmployeeID = ? ) AND (Week = ? )";
            ...
            if (!resultQuery.next()) {
                ...
                char service = resultQuery.getString("Service").charAt(0);
                String expStart = resultQuery.getString("ExpectedEntrance");
                String expEnd = resultQuery.getString("ExpectedExit");
                String decStart = resultQuery.getString("DetectedEntrance");
                String decEnd = resultQuery.getString("DetectedExit");

                return new Shift(expStart, decStart, expEnd, decEnd, day, week, service, employee);
            }
    ...
    /**
     * Get all the days from general turnation, used to determine what is
     * the common week schedule
     */
    ...
    /**
     * Get the weekly schedule of a precise employee in a precise week
     */
...
//------------------//
    ...
    public void deleteEmployee(int employeeID) {
        try {
            ...
            statement = DBMS.prepareStatement(queryString);
            statement.setInt(1, employeeID);
    ...
    public boolean deleteShift() {
        try {
            ...
            queryString = "DELETE FROM Turnation ";
            statement = DBMS.prepareStatement(queryString);
            statement.executeUpdate();
    ...
    public void deleteNotificationAssociated(int employeeID) {
        try {
            ...
            /* Result container */
            ArrayList<Integer> notifList = new ArrayList<>();

            /* First retrieve all the abstention request ID associated
             * with the employee */
            String queryString = "SELECT NotificationID " +
                    "FROM Notification, AbstentionRequest " +
                    "WHERE ReceiverID = ? AND NotificationID = RequestID ";
            ...
            /* Get all the results */
            while (resultQuery.next()) {
                notifList.add(resultQuery.getInt("NotificationID"));
            }

            /* Delete all the requests, it removes the connected notifications */
            for (int  i = 0; i < notifList.size(); ++i) {
                deleteAbstentionRequest(notifList.get(i));
            }
    ...
    public boolean clearAbstentionRequest() {
        try {
            this.enstablishConnection();

            String queryString = "SELECT RequestID " +
            ...
            while (resultQuery.next()) {
                requestList.add(resultQuery.getInt("RequestID"));
            }

            for (int i = 0; i < requestList.size(); ++i) {
                deleteAbstentionRequest(requestList.get(i));
            }
    ...
    public boolean validateHiringAccounts() {
        try {
            this.enstablishConnection();

            String queryString = "DELETE FROM HireQueue ";
    ...
    public boolean deleteFiredAccounts() {
        try {
            this.enstablishConnection();

            String queryString = "SELECT * FROM FireQueue ";

            PreparedStatement statement = DBMS.prepareStatement(queryString);
            resultQuery = statement.executeQuery();

            ArrayList<Integer> firedEmployees = new ArrayList<>();

            while (resultQuery.next()) {
                firedEmployees.add(resultQuery.getInt("EmployeeID"));
            }

            for (int i = 0; i < firedEmployees.size(); ++i) {
                deleteEmployee(firedEmployees.get(i));
                deleteNotificationAssociated(firedEmployees.get(i));
            }
...
//------------------//
    ...
    public boolean modifyShiftExit(String exit, int day, int week, int employeeID) {
        try {
            this.enstablishConnection();
